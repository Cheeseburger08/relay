...
Incoming SMS events now trigger Web Push to the owner's existing notification subscriptions after commit. Replayed events, outgoing SMS and history imports do not notify. Payloads contain SIM, conversation ID, sender name (or number), and the first 500 Unicode characters of the message. SMS notification clicks open the conversation; call notifications retain their call destination.

2026-09-25: Companion 0.8.3 marks new provider SMS with optional `live: true`; only newly imported unread incoming rows notify. Historical imports and retries do not. Authenticated browsers receive `data_changed` signals and refresh immediately. POST `/api/voice/push/status` checks this user's endpoint registration; DELETE `/api/voice/push` removes only this user's endpoint. Push results record content-free acceptance/failure diagnostics, and subscriptions send concurrently.
